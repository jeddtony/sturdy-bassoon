...
//////////// For Job Roles
export type TDataReadJobRoles = {
  limit?: number
  skip?: number
}

export type TDataCreateJobRole = {
  requestBody: JobRoleCreate
}
...
export class JobRoleService {
   ...
   * Read Job Roles
   */
  public static readJobRoles(
    data: TDataReadJobRoles = {},
  ): CancelablePromise<JobRolesPublic> {
    const { limit = 100, skip = 0 } = data
    return __request(OpenAPI, {
      method: "GET",
   ...
   * Create Job Role
   */
  public static createJobRole(
    data: TDataCreateJobRole,
  ): CancelablePromise<JobRolePublic> {
    const { requestBody } = data
    return __request(OpenAPI, {
      method: "POST",
      url: "/api/v1/job-roles/",
